...
def build_page_executive_summary(
    ...
    verdict_with_confidence = (
        get_confidence_prefix(overall_confidence)
        + verdict
        + get_confidence_suffix(overall_confidence)
    )
    ...
    if limiting_factor and limiting_factor != "---":
        elements.append(
            Paragraph(
                f"<font size='9' color='#7F8C8D'>Uwaga: jakość ograniczona przez <b>{limiting_factor}</b></font>",
                styles["body"],
            )
        )
...
def build_page_executive_verdict(
    ...
    hero_content = [
        [
            Paragraph(
                f"<font size='12' color='#FFFFFF'><b>WERDYKT GŁÓWNY</b></font>", styles["center"]
            )
        ],
        [
            Paragraph(
...
def build_page_cover(
    ...
    cell_style = ParagraphStyle(
        "cell", parent=styles["body"], fontSize=9, leading=11, alignment=0, fontName=FONT_FAMILY
    )
    ...
    header_style = ParagraphStyle(
        "header_white", parent=cell_style_bold, textColor=HexColor("#FFFFFF"), alignment=1
    )
...
def build_page_thresholds(
    ...
    data = [
        ...
        [
            "VT1 (Próg wentylacyjny 1)",
            format_thresh(vt1_watts, vt1_range),
            fmt(vt1_hr),
            fmt(vt1_ve),
            fmt(vt1_br),
        ],
        [
            "VT2 (Próg went. 2 / RCP)",
            format_thresh(vt2_watts, vt2_range),
            fmt(vt2_hr),
            fmt(vt2_ve),
            fmt(vt2_br),
        ],
...
# ============================================================================
# PAGE: SmO2 ANALYSIS (PREMIUM MUSCLE OXYGENATION DIAGNOSTIC)
# Moved to smo2.py — re-exported for backward compatibility.
# ============================================================================
from .smo2 import build_page_smo2  # noqa: F401
...
def build_page_pdc(
    ...
    if w_prime_val > 0 and cp_val > 0:
        elements.append(Spacer(1, 6 * mm))
        elements.append(
            Paragraph("<b>REKONSTYTUCJA W' (Caen et al. 2021)</b>", styles["subheading"])
        )
        elements.append(Spacer(1, 2 * mm))
...
# ============================================================================
from .metabolic import build_page_metabolic_engine  # noqa: F401
...
def build_page_limiter_radar(
    limiter_data: Dict[str, Any], figure_paths: Dict[str, str], styles: Dict
) -> List:
    ...
    elements.append(
        Paragraph("<font size='14'>4.1 RADAR OBCIĄŻENIA SYSTEMÓW</font>", styles["center"])
    )
    elements.append(
        Paragraph(
    ...
    if figure_paths and "limiter_radar" in figure_paths:
        elements.extend(
            _build_chart(
                figure_paths["limiter_radar"],
                f"Profil Obciążenia: {window} ({peak_power:.0f} W)",
                styles,
            )
        )
        elements.append(Spacer(1, 6 * mm))
        ...
        TableStyle(
            [
                ...
                (
                    ...
                    (0, 1),
                    (0, 1),
                    HexColor(hr_color) if limiting_factor == "Serce" else HexColor("#F8F9FA"),
                ),
                (
                    "BACKGROUND",
                    (0, 2),
                    (0, 2),
                    HexColor(ve_color) if limiting_factor == "Płuca" else HexColor("#F8F9FA"),
                ),
                (
                    "BACKGROUND",
                    (0, 3),
                    (0, 3),
                    HexColor(smo2_color) if limiting_factor == "Mięśnie" else HexColor("#F8F9FA"),
                ),
            ]
        )
    )
    ...
    limiter_colors = {"Serce": "#E74C3C", "Płuca": "#3498DB", "Mięśnie": "#F39C12"}
    ...
    interp_style = ParagraphStyle(
        "interp", parent=styles["body"], textColor=HexColor("#FFFFFF"), fontSize=9
    )
    interp_box = Table([[Paragraph(interp_content, interp_style)]], colWidths=[170 * mm])
    interp_box.setStyle(
        TableStyle(
            [
...
def build_page_zones(
    thresholds: Dict[str, Any], styles: Dict, training_zones: Optional[Dict[str, Any]] = None
) -> List:
...
def build_page_limitations(styles: Dict, is_conditional: bool = False) -> List:
    ...
    elements.append(
        Paragraph("<font size='14'>5.5 OGRANICZENIA INTERPRETACJI</font>", styles["center"])
    )
    elements.append(Spacer(1, 6 * mm))
...
def build_page_protocol(styles: Dict) -> List:
...
from .thermal import build_page_thermal  # noqa: F401
...
# ============================================================================
# BIOMECHANIKA — extracted to biomech.py
# ============================================================================

from .biomech import build_page_biomech  # noqa: F401 — re-export
...
def build_page_drift(kpi: Dict[str, Any], figure_paths: Dict[str, str], styles: Dict) -> List:
    ...
    if figure_paths and "drift_heatmap_hr" in figure_paths:
        elements.extend(
            _build_chart(figure_paths["drift_heatmap_hr"], "Mapa Dryfu (HR vs Power)", styles)
        )
        elements.append(Spacer(1, 4 * mm))
    ...
    if figure_paths and "drift_heatmap_smo2" in figure_paths:
        elements.extend(
            _build_chart(
                figure_paths["drift_heatmap_smo2"], "Mapa Oksydacji (SmO2 vs Power)", styles
            )
        )
        elements.append(Spacer(1, 4 * mm))
...
def build_page_kpi_dashboard(kpi: Dict[str, Any], styles: Dict) -> List:
    ...
    elements.append(
        Paragraph(
            "<font size='10' color='#7F8C8D'>Dashboard stabilności układu krążenia i kosztu energetycznego</font>",
            styles["body"],
        )
    )
    elements.append(Spacer(1, 8 * mm))
    ...
    def get_ef_status(val):
        ...
        if val is None:
            return ("n/a", "BRAK", "#808080")
        try:
            v = float(val)
            if v < 1.8:
                return (f"{v:.2f}", "SŁABO", "#E74C3C")
            elif v < 2.2:
                return (f"{v:.2f}", "OK", "#27AE60")
            else:
                return (f"{v:.2f}", "BARDZO DOBRZE", "#2ECC71")
        except (ValueError, TypeError):
            return ("n/a", "BRAK", "#808080")
    ...
    def get_pahr_status(val):
        ...
        try:
            v = float(val)
            if v < 5:
                return (f"{v:.1f}%", "STABILNY", "#27AE60")
            elif v < 8:
                return (f"{v:.1f}%", "OSTRZEŻENIE", "#F39C12")
            else:
                return (f"{v:.1f}%", "RYZYKO", "#E74C3C")
        except (ValueError, TypeError):
            return ("n/a", "BRAK", "#808080")
    ...
    def get_smo2_drift_status(val):
        ...
        try:
            ...
            if v_abs < 5:
                ...
                return (f"{v:.1f}%", "UMIARKOWANY DRYF", "#F39C12")
            else:
                return (f"{v:.1f}%", "WYSOKI DRYF OBWODOWY", "#E74C3C")
        except (ValueError, TypeError):
            return ("n/a", "BRAK", "#808080")
    ...
    kpi_cell = ParagraphStyle(
        "kpi_cell", parent=styles["body"], fontSize=9, leading=11, fontName=FONT_FAMILY, alignment=0
    )
    kpi_cell_c = ParagraphStyle("kpi_cell_c", parent=kpi_cell, alignment=1)
    kpi_header = ParagraphStyle(
        "kpi_header",
        parent=kpi_cell,
    ...
    rows = [
        [
            Paragraph("Metryka", kpi_header),
            Paragraph("Wartość", kpi_header),
            Paragraph("Zakres Ref.", kpi_header),
            Paragraph("Status", kpi_header),
        ],
        [
            Paragraph("Efficiency Factor (EF)", kpi_cell),
            Paragraph(ef_val, kpi_cell_c),
            Paragraph("<1.8 słabo | 1.8-2.2 ok | >2.2 b.dobrze", kpi_cell),
            Paragraph(ef_status, kpi_status),
        ],
        [
            Paragraph("Pa:Hr Decoupling", kpi_cell),
            Paragraph(pahr_val, kpi_cell_c),
            Paragraph("<5% stab. | 5-8% ostrz. | >8% ryzyko", kpi_cell),
            Paragraph(pahr_status, kpi_status),
        ],
        [
            Paragraph("SmO₂ Drift", kpi_cell),
            Paragraph(smo2_val, kpi_cell_c),
            Paragraph("<5% stab. | 5-15% umiark. | >15% wysoki", kpi_cell),
            Paragraph(smo2_status, kpi_status),
        ],
        [
            Paragraph("VO₂max", kpi_cell),
            Paragraph(vo2max_val, kpi_cell_c),
    ...
    legend_style = ParagraphStyle(
        "legend", parent=styles["body"], fontSize=8, textColor=HexColor("#95A5A6")
    )
    elements.append(
        Paragraph(
    ...
    pahr_target = calc_target(
        pahr_val.replace("%", ""), 7.5, is_reduction=True
    )  # improve by 5-10 pp → 7.5% reduction
    smo2_target = calc_target(
        smo2_val.replace("%", ""), 35, is_reduction=True
    )  # reduce by 30-40% → 35%
    ...
    elements.append(
        Paragraph("Cele zakładają ustrukturyzowany trening i adaptację cieplną.", disclaimer_style)
    )
...
def build_page_drift_kpi(kpi: Dict[str, Any], figure_paths: Dict[str, str], styles: Dict) -> List:
...
def build_page_limiters(
    metadata: Dict[str, Any], cp_model: Dict[str, Any], figure_paths: Dict[str, str], styles: Dict
) -> List:
    ...
    if figure_paths and "vlamax_balance" in figure_paths:
        elements.extend(
            _build_chart(figure_paths["vlamax_balance"], "Balans VO2max vs VLaMax", styles)
        )
        elements.append(Spacer(1, 4 * mm))
    else:
        elements.append(
            Paragraph("Brak danych do analizy VLaMax (wymagane min. 20 min MMP)", styles["small"])
        )

    # 2. Radar Chart
    if figure_paths and "limiters_radar" in figure_paths:
        elements.extend(
            _build_chart(figure_paths["limiters_radar"], "Radar Obciążenia (5 min Peak)", styles)
        )
        elements.append(Spacer(1, 4 * mm))
...
def build_page_hrv(hrv_data: Dict[str, Any], styles: Dict) -> List:
    ...
    elements.append(
        Paragraph(
            "<font size='14'>3.5 ZMIENNOŚĆ RYTMU SERCA (HRV / DFA Alpha-1)</font>", styles["center"]
        )
    )
    elements.append(
        Paragraph(
...
def build_page_skin_temp(
    ...
    elements.append(
        Paragraph(
            "<font size='14'>4.4 GRADIENT TEMPERATURY (RDZEŃ − SKÓRA)</font>", styles["center"]
        )
    )
    elements.append(
        Paragraph(
...
def build_page_altitude(canonical_data: Dict[str, Any], styles: Dict) -> List:
    ...
    elements.append(
        Paragraph("<font size='14'>2.7 KOREKTA ŚRODOWISKOWA (WYSOKOŚĆ)</font>", styles["center"])
    )
    elements.append(
        Paragraph(
    ...
    vo2max_data = canonical_data.get("canonical_physiology", {}).get(
        "vo2max", {}
    ...
    altitude_levels = [
        ...
        [
            "0 (p.m.)",
            "0%",
            f"{vo2max_sea:.1f} ml/kg/min" if vo2max_sea > 0 else "---",
            "Poziom morza — wartość referencyjna",
        ],
        [
            "500",
            "3.2%",
            f"{vo2max_sea * 0.968:.1f}" if vo2max_sea > 0 else "---",
            "Minimalna hipoksja",
        ],
        [
            "1000",
            "6.3%",
            f"{vo2max_sea * 0.937:.1f}" if vo2max_sea > 0 else "---",
            "Kraków, Monachium",
        ],
        [
            "1500",
            "9.5%",
            f"{vo2max_sea * 0.905:.1f}" if vo2max_sea > 0 else "---",
            "Livigno, Font Romeu",
        ],
        [
            "2000",
            "12.6%",
            f"{vo2max_sea * 0.874:.1f}" if vo2max_sea > 0 else "---",
            "Sierra Nevada, Flagstaff",
        ],
        [
            "2500",
            "15.8%",
            f"{vo2max_sea * 0.842:.1f}" if vo2max_sea > 0 else "---",
            "La Paz, Bogota",
        ],
    ]
...
def build_page_microcycle(metabolic_data: Dict[str, Any], styles: Dict) -> List:
    ...
    elements.append(
        Paragraph("<font size='14'>2.5 PERIODYZACJA MIKROCYKLOWA</font>", styles["center"])
    )
    elements.append(
        Paragraph(
            "<font size='10' color='#7F8C8D'>"
    ...
    for s in sessions:
        ...
        if any(
            k in name_lower
            for k in ["vo2", "hill", "sprint", "over-under", "frc", "threshold", "race"]
        ):
    ...
    for day_idx, session in enumerate(weekly_plan):
        ...
        if session is None:
            if day_idx == 0:  # Monday = rest
                ...
                week_data.append(
                    [day_name, "Z2 Długie lub ODPOCZYNEK", "2-4h @ 55-70% FTP", "Z1-Z2"]
                )
            else:

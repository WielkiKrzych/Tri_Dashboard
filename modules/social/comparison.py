...
from modules.social.reference_data import (
    get_reference_table,
    get_age_bracket,
...
@dataclass
class BenchmarkPercentileRanking:
    metric_name: str
    value: float
...
class BenchmarkService:
    """Compute percentile rankings vs population reference data."""

    def get_cp_percentile(
        self, cp_watts: float, weight_kg: float, age: int, gender: str
    ) -> BenchmarkPercentileRanking:
        cp_wkg = cp_watts / weight_kg if weight_kg > 0 else 0
        ref = get_reference_table("cp_wkg", gender, age)
        pct = interpolate_percentile(cp_wkg, ref)
        return BenchmarkPercentileRanking(
            metric_name="CP (W/kg)",
            value=cp_watts,
            value_normalized=cp_wkg,
            percentile=pct,
            category=classify_percentile(pct),
            age_bracket=get_age_bracket(age),
        )

    def get_vo2max_percentile(
        self, vo2max: float, age: int, gender: str
    ) -> BenchmarkPercentileRanking:
        ref = get_reference_table("vo2max", gender, age)
        pct = interpolate_percentile(vo2max, ref)
        return BenchmarkPercentileRanking(
            metric_name="VO2max (mL/kg/min)",
            value=vo2max,
            value_normalized=vo2max,
            percentile=pct,
            category=classify_percentile(pct),
            age_bracket=get_age_bracket(age),
        )

    def get_wprime_percentile(
        self, w_prime_j: float, weight_kg: float, age: int, gender: str
    ) -> BenchmarkPercentileRanking:
        wprime_jkg = w_prime_j / weight_kg if weight_kg > 0 else 0
        ref = get_reference_table("wprime_jkg", gender, age)
        pct = interpolate_percentile(wprime_jkg, ref)
        return BenchmarkPercentileRanking(
            metric_name="W' (J/kg)",
            value=w_prime_j,
            value_normalized=wprime_jkg,
            percentile=pct,
            category=classify_percentile(pct),
            age_bracket=get_age_bracket(age),
        )
    ...
    def get_full_benchmark(
        ...
        age: int,
        gender: str,
    ) -> list[BenchmarkPercentileRanking]:
        rankings: list[BenchmarkPercentileRanking] = []
        if weight_kg > 0 and cp_watts > 0:
            rankings.append(self.get_cp_percentile(cp_watts, weight_kg, age, gender))
        if vo2max > 0:
            rankings.append(self.get_vo2max_percentile(vo2max, age, gender))
        if weight_kg > 0 and w_prime_j > 0:
            rankings.append(self.get_wprime_percentile(w_prime_j, weight_kg, age, gender))
        return rankings

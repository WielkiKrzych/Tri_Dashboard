...
def detect_vt1_peaks_heuristic(
    ...
    """
    Heuristic VT1 Detection:
    1. Smooth VE.
    2. Find first 2 'noticeable' peaks.
    3. Check Trend (Slope) between Peak 1 and Peak 2.
    4. If Slope > 0.05 -> VT1 is at Peak 2 (or end of segment).
    """
    notes = []
    
    # 1. Smooth VE (strong smoothing to find 'noticeable' peaks)
    # Using ~30s window or 5% of data? Let's use 30s as default
    if len(df) < 120: 
    ...
    # 2. Find Peaks
    # prominence=2 ensures they are distinct 'hills' roughly > 2 L/min local drop
    # distance=30 ensures they are separated by ~30 samples (seconds)
    peaks, properties = signal.find_peaks(ve_smooth, distance=60, prominence=1.5)
    ...
    # 3. Analyze Trend between P1 and P2
    # Slice data between peak 1 and peak 2
    ...
    if slope > 0.05:
        # Success!
        # Return metrics extracted from the segment (average) or the second peak?
        # "Trend ... ma wtedy wynosić > 0.05".
        # If valid, let's take the properties at the END of the trend (Peak 2) or the Segment Avg?
        # Usually thresholds are marked *at* the breakpoint. 
        # User screenshot shows "MANUAL" range. 
        # Let's map it to the average of the segment, as that represents the "Trend Zone".
        ...
        return {
            ...
            # For BR we need to check columns inside the calling function or pass proper name
            'idx_end': p2_idx
...
def detect_vt_from_steps(
    ...
    # 1. Prepare Stages (Whole Steps)
    # Filter out first step
    stages = []
...
def detect_smo2_from_steps(
    ...
    - Use whole steps (180s)
    - LT1: First step (after index 2) where slope < -0.01
    - LT2: Next step after LT1 where slope < -0.02
    """
    ...
    # 1. Collect analysis for whole steps
    all_steps = []
    
    # User: Skip first two steps
    for step in step_range.steps[2:]: 
        step_mask = (df[time_column] >= step.start_time) & (df[time_column] < step.end_time)
        step_data = df[step_mask]
        
        if len(step_data) < 10:
            continue
            
        slope, _, _ = calculate_slope(step_data[time_column], step_data[smo2_column])
        ...
        info = {
            'step_number': step.step_number,
            'sub_step': 0, # Whole step
            'start_time': step_data[time_column].min(),
            'end_time': step_data[time_column].max(),
    ...
    # Find LT1: First item with slope < -0.01
    ...
    if lt1_found:
        # Find LT2: NEXT item with slope < -0.02
        for i in range(lt1_idx + 1, len(all_steps)):
            item = all_steps[i]
            if item['slope'] < -0.02:
                item['is_t2'] = True
                result.smo2_2_watts = item['avg_power']
                result.smo2_2_hr = item['avg_hr']
                result.smo2_2_step_number = item['step_number']
                result.smo2_2_slope = item['slope']
                break

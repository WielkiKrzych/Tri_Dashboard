...
def render_manual_thresholds_tab(
    target_df, training_notes, uploaded_file_name, cp_input, max_hr_input
):
    ...
    with col_inp1:
        manual_vt1 = st.number_input(
            "Manualny VT1 (Moc W)",
            min_value=0,
            max_value=1000,
            value=int(result.vt1_watts) if result.vt1_watts else 0,
            step=5,
            key="manual_vt1_watts",
        )
    with col_inp2:
        manual_vt2 = st.number_input(
            "Manualny VT2 (Moc W)",
            min_value=0,
            max_value=1000,
            value=int(result.vt2_watts) if result.vt2_watts else 0,
            step=5,
            key="manual_vt2_watts",
        )
    ...
    with col_inpa:
        ...
        manual_vt1_hr = st.number_input(
            "VT1 HR (bpm)",
            min_value=0,
            max_value=250,
            value=int(vt1_hr_est) if vt1_hr_est else 0,
            step=1,
            key="vt1_hr",
        )
        manual_vt1_ve = st.number_input(
            "VT1 VE (L/min)",
            min_value=0.0,
            max_value=300.0,
            value=float(result.vt1_ve) if result.vt1_ve else 0.0,
            step=1.0,
            key="vt1_ve",
        )
        manual_vt1_br = st.number_input(
            "VT1 Oddechy (BR/min)",
            min_value=0,
            max_value=120,
            value=int(result.vt1_br) if result.vt1_br else 0,
            step=1,
            key="vt1_br",
        )
    ...
    with col_inpb:
        ...
        manual_vt2_hr = st.number_input(
            "VT2 HR (bpm)",
            min_value=0,
            max_value=250,
            value=int(vt2_hr_est) if vt2_hr_est else 0,
            step=1,
            key="vt2_hr",
        )
        manual_vt2_ve = st.number_input(
            "VT2 VE (L/min)",
            min_value=0.0,
            max_value=300.0,
            value=float(result.vt2_ve) if result.vt2_ve else 0.0,
            step=1.0,
            key="vt2_ve",
        )
        manual_vt2_br = st.number_input(
            "VT2 Oddechy (BR/min)",
            min_value=0,
            max_value=120,
            value=int(result.vt2_br) if result.vt2_br else 0,
            step=1,
            key="vt2_br",
        )
            ...
            **Histereza** to różnica między progami wykrytymi podczas wzrostu obciążenia (Ramp Up) 
            ...
            with h_col1:
                if hysteresis.vt1_dec_zone:
                    st.markdown(
                        f"**VT1 (Down):** {hysteresis.vt1_dec_zone.range_watts[0]:.0f} - {hysteresis.vt1_dec_zone.range_watts[1]:.0f} W"
                    )
            ...
            with h_col2:
                if hysteresis.vt2_dec_zone:
                    st.markdown(
                        f"**VT2 (Down):** {hysteresis.vt2_dec_zone.range_watts[0]:.0f} - {hysteresis.vt2_dec_zone.range_watts[1]:.0f} W"
                    )
    ...
    if manual_vt1 > 0 and vt1_time_manual is not None:
        fig_thresh.add_vline(
            x=vt1_time_manual, line=dict(color="#ffa15a", width=3, dash="dash"), layer="above"
        )
    ...
    if manual_vt2 > 0 and vt2_time_manual is not None:
        fig_thresh.add_vline(
            x=vt2_time_manual, line=dict(color="#ef553b", width=3, dash="dash"), layer="above"
        )
        fig_thresh.add_annotation(
            x=vt2_time_manual,
            y=1,
    ...
    if hysteresis:
        if hysteresis.vt1_dec_zone:
            fig_thresh.add_hrect(
                y0=hysteresis.vt1_dec_zone.range_watts[0],
                y1=hysteresis.vt1_dec_zone.range_watts[1],
                fillcolor="blue",
                opacity=0.05,
                layer="below",
                line_width=1,
                ...
                line_color="blue",
                yref="y2",
                annotation_text="VT1 (Down)",
                annotation_position="bottom right",
            )

        if hysteresis.vt2_dec_zone:
            fig_thresh.add_hrect(
                y0=hysteresis.vt2_dec_zone.range_watts[0],
                y1=hysteresis.vt2_dec_zone.range_watts[1],
                fillcolor="purple",
                opacity=0.05,
                layer="below",
                line_width=1,
        ...
        with st.expander("📊 Analiza Wrażliwości (Sensitivity)", expanded=False):
            ...
            **Sensitivity Analysis** sprawdza, jak stabilne są wykryte progi przy różnych parametrach algorytmu.
            ...
            with s_col1:
                st.metric(
                    "VT1 Stability Score",
                    f"{sensitivity.vt1_stability_score:.2f}",
            ...
            with s_col2:
                st.metric(
                    "VT2 Stability Score",
                    f"{sensitivity.vt2_stability_score:.2f}",
                    help="0-1, wyżej = lepiej",
                )
    ...
    with col_z1:
        zone_vt2 = st.number_input(
            "VT2 Power (W)",
            min_value=0,
            max_value=1000,
            value=int(manual_vt2) if manual_vt2 > 0 else 250,
            step=5,
            help="100% mocy dla wyliczenia stref",
        )
    with col_z2:
        zone_lthr = st.number_input(
            "LTHR (HR na VT2)",
            min_value=0,
            max_value=250,
            value=int(manual_vt2_hr) if manual_vt2_hr > 0 else 170,
            step=1,
            help="100% tętna progowego (VT2)",
        )
    with col_z3:
        zone_max_hr = st.number_input(
            "Max Heart Rate (bpm)",
            min_value=0,
            max_value=250,

...
def render_vent_thresholds_tab(
    ...
    if pending_result is not None:
        ...
        if can_generate and st.button(
            "📄 GENERUJ RAPORT", type="primary", use_container_width=True
        ):
        ...
        if df_s is not None and len(df_s) > 0:
            ...
            if has_ve_vo2 or has_ve_vco2:
                ...
                fig, axes = plt.subplots(
                    1, 2 if (has_ve_vo2 and has_ve_vco2) else 1, figsize=(14, 5)
                )
                ...
                if has_ve_vo2:
                    ...
                    if v1_w:
                        ...
                        if len(vt1_row) > 0:
                            ...
                            ax1.scatter(
                                [v1_w], [y_vt1], color="#ffa15a", s=150, zorder=5, marker="*"
                            )
                ...
                if has_ve_vco2:
                    ...
                    if v2_w:
                        ...
                        if len(vt2_row) > 0:
                            ...
                            ax2.scatter(
                                [v2_w], [y_vt2], color="#ef553b", s=150, zorder=5, marker="*"
                            )
    ...
    if vt1_onset_t is not None:
        ...
        fig_thresh.add_vrect(
            x0=target_df["time"].min(),
            x1=vt1_onset_t,
            fillcolor=zone_colors["z1"],
            layer="below",
            line_width=0,
        )
    ...
    if rcp_onset_t:
        ...
        fig_thresh.add_vrect(
            x0=rcp_onset_t,
            x1=target_df["time"].max(),
            fillcolor=zone_colors["z4"],
            layer="below",
            line_width=0,
        )
        ...
        if time is not None and power is not None:
            ...
            if name == "vt1_steady" and is_vt1_steady_interpolated:
                ...
                fig_thresh.add_vline(
                    x=time, line=dict(color="#666", width=2, dash="dot"), layer="above"
                )
                ...
                fig_thresh.add_vline(
                    x=time, line=dict(color=color, width=2, dash="dash"), layer="above"
                )

...
from modules.calculations.column_aliases import HR_ALIASES, POWER_ALIASES
...
def render_thermal_tab(df_plot):
    ...
    if fig_t is not None:
        st.plotly_chart(fig_t, width="stretch", config=CHART_CONFIG)
    ...
    hr_aliases = ("hr", "heartrate_smooth", *HR_ALIASES, "pulse")
    pwr_aliases = ("watts", "watts_smooth", *POWER_ALIASES, "pwr", "moc")
    ...
    if pwr_col and hr_col and temp_col:
        ...
        if not df_clean.empty:
            ...
            st.plotly_chart(fig_te, width="stretch", config=CHART_CONFIG)
...
def _render_thermal_prediction_section(df_plot, decay_result):
    ...
    hr_threshold = c4.number_input(
        "HR @ próg [bpm]", min_value=120, max_value=200, value=165, step=5
    )

    c5, c6 = st.columns(2)
    # Use detected decay if available, otherwise default
    default_decay = (
        decay_result.get("decay_pct_per_c", -3.0)
        if decay_result.get("is_significant", False)
        else -3.0
    )
    decay_pct = c5.slider(
        "Spadek wydajności [%/°C]",
        min_value=-10.0,
        max_value=0.0,
        value=float(default_decay),
        step=0.5,
    )
    hr_increase = c6.slider(
        "Wzrost HR [bpm/°C]", min_value=1.0, max_value=10.0, value=3.0, step=0.5
    )

    # === TEMPERATURA DOCELOWA ===
    st.subheader("🎯 Temperatura Startu / Scenariusz")
    target_temp = st.slider(
        "Temperatura rdzenia [°C]", min_value=37.0, max_value=41.0, value=39.0, step=0.1
    )
    ...
    fig_cp.add_vline(
        ...
        line_dash="dot",
        line_color="red",
        annotation_text=f"{target_temp}°C",
    ...
    st.plotly_chart(fig_cp, width="stretch", config=CHART_CONFIG)

...
def generate_radar_chart(
    ...
    if source_df is None or source_df.empty:
        fig = create_empty_figure("Brak danych źródłowych", "Profil Metaboliczny", config)
    ...
    if not pwr_col:
        fig = create_empty_figure("Brak danych Mocy", "Profil Metaboliczny", config)
        return save_figure(fig, config, output_path)

    # Rolling 5min (300s)
    window_sec = 300
    # Assuming standard sampling ~1Hz. If time column available, could differ, but simplified:
    # Most devices export 1Hz.
    df['rolling_watts'] = df[pwr_col].rolling(window=window_sec, min_periods=window_sec).mean()
    
    if df['rolling_watts'].isna().all():
        fig = create_empty_figure("Za krótki trening (<5min)", "Profil Metaboliczny", config)
        return save_figure(fig, config, output_path)
    ...
    # Try to get VT2 VE from report, or estimate
    vt2_ve = 0
    try:
        thresholds = report_data.get("thresholds", {})
        vt2 = thresholds.get("vt2_result", thresholds.get("vt2", {})) # handle different structures
        vt2_ve = vt2.get("ve", 0)
    except:
        pass
    
    # If VT2 VE missing, use max from file
    if ve_col:
    ...
    fig, ax = plt.subplots(figsize=config.figsize, dpi=config.dpi, subplot_kw=dict(polar=True))
    ...
    ax.set_xticklabels(categories, fontsize=config.font_size)
    ...
    ax.set_title(f"Profil Obciążenia (5 min @ {peak_w_avg:.0f} W)", 
                 fontsize=config.title_size, fontweight='bold', pad=20)
    
    # Add values annotation
    # (Matplotlib polar annotations are tricky, skipping for simplicity or adding simple text box)

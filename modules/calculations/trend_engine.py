...
# =============================================================================
# Statistical Trend Analysis (F-02: Longitudinal Trend Dashboard)
# =============================================================================


@dataclass
class StatisticalTrend:
    """Full statistical trend with confidence intervals and projections."""
    ...
    ci_upper: float
    n_points: int
    projection_4w: Optional[float]
    projection_8w: Optional[float]
...
def calculate_statistical_trend(
    ...
    slope, intercept, r_value, p_value, std_err = scipy_stats.linregress(arr_days, arr)
    r_squared = r_value**2

    mean_val = np.mean(arr)
    slope_pct = (slope * 7 / mean_val * 100) if mean_val != 0 else 0.0

    t_crit = scipy_stats.t.ppf(0.975, df=n - 2) if n > 2 else 1.96
    ci_lo = ((slope - t_crit * std_err) * 7 / mean_val * 100) if mean_val != 0 else 0.0
    ci_hi = ((slope + t_crit * std_err) * 7 / mean_val * 100) if mean_val != 0 else 0.0

    last_val = arr[-1]
    proj_4w = last_val + slope_pct * 4 if p_value < 0.05 else None
    proj_8w = last_val + slope_pct * 8 if p_value < 0.05 else None

    return StatisticalTrend(
        name=name,
        slope_per_week=slope * 7,
        slope_pct_per_week=slope_pct,
        r_squared=r_squared,
        p_value=p_value,
        ...
        ci_upper=ci_hi,
        n_points=n,
        projection_4w=proj_4w,
        projection_8w=proj_8w,
    )


def extract_vo2max_from_report(report: dict[str, Any]) -> float:
    """Extract VO2max from ramp test report JSON."""
    paths = [
        ("canonical_physiology", "summary", "vo2max"),
        ("physiological_markers", "vo2max_ml_kg_min"),
        ("vo2max_estimated"),
        ("cp_model", "vo2max"),
    ]
    for path in paths:

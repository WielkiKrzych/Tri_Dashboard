...
def generate_biomech_chart(
    ...
    if source_df is not None and not source_df.empty:
        ...
        if "power" in df.columns and "cad_raw" in df.columns:
            ...
            # Calculate Torque (vectorized)
            angular_velocity = df["cadence"] * 2 * np.pi / 60
            df["torque"] = np.where(df["cadence"] >= 60, df["power"] / angular_velocity, 0)
...
def generate_torque_smo2_chart(
    ...
    # Calculate Torque (vectorized)
    angular_velocity = df_calc["cadence"] * 2 * np.pi / 60
    df_calc["torque"] = np.where(df_calc["cadence"] >= 60, df_calc["power"] / angular_velocity, 0)

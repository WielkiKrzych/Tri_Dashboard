...
def render_smo2_manual_thresholds_tab(target_df, training_notes, uploaded_file_name, cp_input):
    ...
    st.markdown(
        "Wprowadź własne wartości mocy dla progów metabolicznych SmO2, aby zobaczyć je na wykresie."
    )
    ...
    if not proto_check["is_valid"]:
        st.warning(
            "⚠️ Protokół może nie być idealnym testem schodkowym. Wyniki mogą być przybliżone."
        )
    else:
    ...
    with col_inp1:
        manual_lt1 = st.number_input(
            "Manualny LT1 (Moc W)",
            min_value=0,
            max_value=1000,
            value=int(result.smo2_1_watts) if result.smo2_1_watts else 0,
            step=5,
            key="smo2_lt1_m",
        )
    with col_inp2:
        manual_lt2 = st.number_input(
            "Manualny LT2 (Moc W)",
            min_value=0,
            max_value=1000,
            value=int(result.smo2_2_watts) if result.smo2_2_watts else 0,
            step=5,
            key="smo2_lt2_m",
        )
    ...
    with col_inpa:
        ...
        manual_lt1_hr = st.number_input(
            "LT1 HR (bpm)",
            min_value=0,
            max_value=250,
            value=int(lt1_hr_est) if lt1_hr_est else 0,
            step=1,
            key="smo2_lt1_hr_m",
        )
        manual_lt1_smo2 = st.number_input(
            "LT1 SmO2 (%)",
            min_value=0.0,
            max_value=100.0,
            value=float(lt1_smo2_est) if lt1_smo2_est else 0.0,
            step=0.5,
            key="smo2_lt1_smo2_m",
        )
    ...
    with col_inpb:
        ...
        manual_lt2_hr = st.number_input(
            "LT2 HR (bpm)",
            min_value=0,
            max_value=250,
            value=int(lt2_hr_est) if lt2_hr_est else 0,
            step=1,
            key="smo2_lt2_hr_m",
        )
        manual_lt2_smo2 = st.number_input(
            "LT2 SmO2 (%)",
            min_value=0.0,
            max_value=100.0,
            value=float(lt2_smo2_est) if lt2_smo2_est else 0.0,
            step=0.5,
            key="smo2_lt2_smo2_m",
        )
    ...
    if manual_lt1 > 0 and lt1_time_manual is not None:
        fig_thresh.add_vline(
            x=lt1_time_manual, line=dict(color="#2ca02c", width=3, dash="dash"), layer="above"
        )
    ...
    if manual_lt2 > 0 and lt2_time_manual is not None:
        fig_thresh.add_vline(
            x=lt2_time_manual, line=dict(color="#d62728", width=3, dash="dash"), layer="above"
        )
        fig_thresh.add_annotation(
            x=lt2_time_manual,
            y=1,
